...
def experimento(exp):
    ...
    for dataset in tqdm(config.datasets,total=len(config.datasets),ncols=70,leave=True,position=0):
        ...
        elif 'HiSum' in dataset:
            batch_size = 1 if config.batch_size=='1' else int(config.HiSum_len*0.8*float(config.batch_size))
        ...
        for split_id,(train_loader,test_loader) in tqdm(enumerate(create_dataloader(dataset)),total=5,ncols=70,leave=False,position=1,desc=dataset):
            ...
            for epoch in tqdm(range(config.epochs),total=config.epochs,ncols=70,leave=False,position=2,desc=f'Split{split_id+1}'):
                ...
                for feature,gtscore,dataset_name,video_num, pascore in tqdm(train_loader,ncols=70,leave=False,position=3,desc=f'Epoch{epoch+1}_TRAIN'):
                    feature = feature.to(config.device)
                    gtscore = gtscore.to(config.device)
                    pascore = pascore.to(config.device)
                ...
                model.eval()
                with torch.no_grad():
                    for feature,gtscore,dataset_name,video_num, pascore in tqdm(test_loader,ncols=70,leave=False,position=3,desc=f'Epoch{epoch+1}_TEST'):
                        feature = feature.to(config.device)
                        gtscore = gtscore.to(config.device)
                        pascore = pascore.to(config.device)
                        output = model(feature)
                ...
                if np.mean(val_kendalls) > model_selection_kendall and np.mean(val_spears) > model_selection_spear:
                    ...
                    print("spear", model_selection_spear)
                    print("kendall", model_selection_kendall)
                    torch.save(model.state_dict(), './tmp/weight.pt')
            os.makedirs(f'./weights/{exp}/{dataset}', exist_ok=True)
            shutil.move('./tmp/weight.pt', f'./weights/{exp}/{dataset}/split{split_id+1}.pt')

if __name__ == '__main__':
    exps = ['hl', 'hlpa', 'hlunpa']
    for exp in exps:
        print("Dataset: {}".format( exp))
        experimento(exp)