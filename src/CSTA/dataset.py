...
def load_h5(videos,data_path,dataset_name):
    ...
    with h5py.File(data_path,'r') as hdf:
        for video in videos:
            ...
            pascore = hdf[video]['pascore'][()]
            features.append(feature)
            gtscores.append(gtscore)
            pascores.append(pascore)
            dataset_names.append(dataset_name)
    return features,gtscores,dataset_names, pascores

# Create Dataset
class VSdataset(Dataset):
    def __init__(self,data,video_nums,transform=None):
        features,gtscores,dataset_names, pascores = data
        self.features = features
        self.gtscores = gtscores
        self.pascores = pascores
    ...
    def __getitem__(self,idx):
        ...
        return torch.unsqueeze(output_feature,0),torch.from_numpy(self.gtscores[idx]).float(),self.dataset_names[idx],self.video_nums[idx], torch.from_numpy(self.pascores[idx]).float().to('cuda')
