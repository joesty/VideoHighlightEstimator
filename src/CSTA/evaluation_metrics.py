...
def get_corr_coeff(pred_imp_scores, videos, dataset, user_scores=None):
    ...
    if dataset=='SumMe':
        ...
        for pred_imp_score,video in zip(pred_imp_scores,videos):
            pred_imp_score = np.squeeze(pred_imp_score).tolist()
            #rho_coeff.append(spearmanr(pred_imp_score,user_scores)[0])
            #tau_coeff.append(kendalltau(rankdata(pred_imp_score),rankdata(user_scores))[0])
            rho_coeff.append(spearmanr(pred_imp_score,user_scores))
            tau_coeff.append(kendalltau(rankdata(pred_imp_score),rankdata(user_scores)))
